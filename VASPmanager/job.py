...
class job(object):
    ...
    archivepath=""
    archivepathdepth=3
    ## [1,2] 1=nameFROM 2=NameTO
    archivefiles=[["POSCAR","POSCAR"],["mist","log"],["PARCHG","PARCHG"],
                            ["CONTCAR","CONTCAR"],["INCAR","INCAR"],["KPOINTS","KPOINTS"],
                            ["OSZICAR","OSZICAR"],["OUTCAR","OUTCAR"],
                            ["vasprun.xml","vasprun.xml"],
                            ["CHGCAR","CHGCAR"],
                            ["PROCAR","PROCAR"],
                            ["EIGENVAL","EIGENVAL"]]
    inputfiles=["INCAR","KPOINTS","POSCAR","CHGCAR","POSCAR2"]
    ...
    def copyinput(self,jobnr):
        ...
        for filename in self.inputfiles:
            self.copymove(jobnr,self.workpath+filename,path+filename) 
        if self.useoldFiles:
    ...
    def copyresult(self,jobnr,runnr):        
        ...
        if self.testrun: 
            ...
            if self.archivepath!="":
                ...
                print "archivepath "+self.archivepath+("/".join(filter(None,savepath.split('/'))[-self.archivepathdepth:]))+"/"
            return True
        print "speichere unter: "+savepath
        ...
        if self.archivepath != "":
            archpath = self.archivepath+"/".join(filter(None,savepath.split('/'))[-self.archivepathdepth:])+"/"
            print "archive: "+archpath
            for archivefile in self.archivefiles:
                self.deletehigher(jobnr,archpath+archivefile[1],runnr)
                self.copymove(jobnr,path+archivefile[0],archpath+archivefile[1],move=False)
            if self.copyWAVECARback:
                self.deletehigher(jobnr,archpath+"WAVECAR",runnr)
                self.copymove(jobnr,path+"WAVECAR",archpath+"WAVECAR",move=False)
    ...
    def startjob(self,jobnr):
        ...
        print "Name: "+fullname+" in Ordner: "+path
