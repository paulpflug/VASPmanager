...
# to access a single number in a matrix look at the following.
# This will make three calculations with the z-component of the third
# basis vector going trough 1, 2 and 3
parameters=[[vm.par.Basisvectors,[1,2,3],3,3]]
```

The optional ```parameternames``` requires only a list of names, if not provided, a name is generated by the first entries of ```parameters``` and ```parameters2```. In the case of ```parameters``` and ```parameters2``` a list of lists is required. One parameter element consists of up to 4 pieces. First the parameter name (vm.par.Basisvectors), then a list of values, which will be used for the different calculations and two optional numbers to access a single number within a vector or a matrix.
